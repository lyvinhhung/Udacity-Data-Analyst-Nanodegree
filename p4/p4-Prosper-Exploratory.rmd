...
prosper$LoanOriginationDate.year <-prosper$LoanOriginationDate %>% format('%Y') %>% strtoi()
...
ggpairs(prosper.sample[ ,c("LenderYield",
        ...
        diag=list(continuous="density", discrete="bar"), axisLabels="none")
```
The graph is a good start for trying to paire these variables together. I noticed a very strong linear relationship between Lender Yield and Borrower Rate. It's almost a straight line.
...
```{r echo=FALSE, Yearly_Income_And_Prosper_Rating}
...
```{r echo=FALSE, Employment_Status_Loan_OriginalAmount}
...
```{r echo=FALSE, BorrowerRate_DebtToIncomeRatio}
...
```{r echo=FALSE, LenderYield_Rating}
...
```{r echo=FALSE, LenderYield_ListingCategory}
...
```{r echo=FALSE, LoanOriginalAmount_Term}
...
```{r echo=FALSE, LenderYield_ProsperRate_Term}
...
```{r echo=FALSE, BorrowerRate_Year_Term}
